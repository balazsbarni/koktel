...
function selectCocktails (alcohol){
  var list = [];
  for (let i in cocktails){
    if (cocktails[i].contains.indexOf(alcohol) != -1){
...
app.get('/', (req, res) => {
  ...
  if (req.query.alcohol != undefined){
    var cocktailList = selectCocktails(req.query.alcohol);
    data.cocktails = cocktailList;
  } else {}
  res.render('cocktail', {data: data});
});
